...
{
    public const string SDK_ID = "HODZVZF4EQPZWYLDOQOKMLJJVIE8FYVH";
...
public struct SDKResolution
{
    ...
    public static SDKResolution zero => new SDKResolution {width = 0, height = 0};
...
{
    public float x, y, z;
    public static SDKVector3 zero => new SDKVector3 {x = 0, y = 0, z = 0};

    public static SDKVector3 one => new SDKVector3 {x = 1, y = 1, z = 1};

    public static SDKVector3 forward => new SDKVector3 {x = 0, y = 0, z = 1};

    public static SDKVector3 up => new SDKVector3 {x = 0, y = 1, z = 0};

    public static SDKVector3 right => new SDKVector3 {x = 1, y = 0, z = 0};
...
public struct SDKQuaternion
{
    ...
    public static SDKQuaternion identity => new SDKQuaternion {x = 0, y = 0, z = 0, w = 1.0f};
    ...
    {
        var tx = lhs.w * rhs.x + lhs.x * rhs.w + lhs.y * rhs.z - lhs.z * rhs.y;
        var ty = lhs.w * rhs.y + lhs.y * rhs.w + lhs.z * rhs.x - lhs.x * rhs.z;
        var tz = lhs.w * rhs.z + lhs.z * rhs.w + lhs.x * rhs.y - lhs.y * rhs.x;
        var tw = lhs.w * rhs.w - lhs.x * rhs.x - lhs.y * rhs.y - lhs.z * rhs.z;
...
public struct SDKMatrix4x4
{
    ...
    public static SDKMatrix4x4 Perspective(float vFov, float aspect, float zNear, float zFar)
    {
        ...
        var hFovRad = 2.0f * Mathf.Atan(Mathf.Tan(vFovRad * 0.5f) * aspect);
        var w = 1.0f / Mathf.Tan(hFovRad * 0.5f);
        var h = 1.0f / Mathf.Tan(vFovRad * 0.5f);
        var q0 = (zFar + zNear) / (zNear - zFar);
        var q1 = 2.0f * (zFar * zNear) / (zNear - zFar);
...
public struct SDKPlane
{
    ...
    public static SDKPlane empty => new SDKPlane {distance = 0f, normal = SDKVector3.up};
...
public struct SDKInputFrame
{
    ...
    public ulong
        frameid; // This is actually the time stamp of this frame - its populated by the bridge at creation time.

    public ulong
        referenceframe; // Use the previous frames frameid to populate this field - it must be set to the correct frame id, or it will fail. 

    public SDKPriority
        priority; // this is a mixed field combining flags and priority - the contents of this flag are not yet set in stone            
